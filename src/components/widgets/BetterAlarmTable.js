...
export default class BetterAlarmTable extends React.Component {
    ...
    render() {
        ...
        const columns = [{
            ...
            Cell: row => (<div><div className="datum">{(row.value > 0) ? moment(row.value).format('DD.MM.YYYY') : "--"}</div><div className="uhrzeit">{(row.value > 0) ? moment(row.value).format('HH:mm:ss') : ""}</div></div>),
            width: 100,
            ...
            Header: 'Quittieren',
            accessor: 'varName',
            Cell: row => (<Button onClick={() => { this.quitAlarm(row.value) }}>Quittieren</Button>),
        ...
        return (
            <ons-col>
                <ons-list>
                    <ons-list-header>
                        <span className="right lastupdate" style={{ float: 'right', paddingRight: '5px' }}>{moment(this.props.state.ts).format('LLL')}</span>
                    </ons-list-header>
                    {title}
                    <ons-list-item>
                        <ReactTable
